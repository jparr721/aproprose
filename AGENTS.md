...
- **No ad-hoc type at call sites**: no `text-[Npx]`, no `tracking-[...]`, no font-family overrides (`font-serif` / `font-heading` / `font-mono` / `font-sans`), no `italic`. The component and the Tailwind type scale (`text-xs`, `text-sm`, ...) own it - the font family is set centrally, never sprinkled onto panels or elements.
