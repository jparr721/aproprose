...
describe("toggleInlineWrap", () => {
  it("wraps a selection and keeps the inner selected", () => {
    ...
    expect(toggleInlineWrap("abc", 1, 2, "**")).toEqual({ text: "a**b**c", start: 3, end: 4 });
  });

  it("wraps with the italic marker", () => {
    expect(toggleInlineWrap("abc", 1, 2, "_")).toEqual({ text: "a_b_c", start: 2, end: 3 });
  });

  it("unwraps when the markers are inside the selection", () => {
    // select "**b**" (1..6)
    expect(toggleInlineWrap("a**b**c", 1, 6, "**")).toEqual({ text: "abc", start: 1, end: 2 });
  });

  it("unwraps when the markers sit just outside the selection", () => {
    // select "b" (3..4) of "a**b**c"
    expect(toggleInlineWrap("a**b**c", 3, 4, "**")).toEqual({ text: "abc", start: 1, end: 2 });
  });
