...
// and routes replacements back through it (one undo step each).
...
interface FindState extends FindOptions {
  ...
  toggleRegex: () => void;
  toggleReplace: () => void;
  /** Re-derive matches from the live blocks; clamps the current index. DOM-free. */
  recompute: () => void;
  /** Center the active match in the viewport - called on explicit nav / search change. */
...
function optionsOf(s: FindState): FindOptions {
  return { caseSensitive: s.caseSensitive, wholeWord: s.wholeWord, regex: s.regex };
}
...
export const useFindStore = create<FindState>((set, get) => {
  // next/prev are the same wrap-around step in opposite directions.
  const step = (delta: 1 | -1): void => {
    const s = get();
    const n = s.matches.length;
    if (n === 0) return;
    const currentIndex = (s.currentIndex + delta + n) % n;
    set({ currentIndex });
  ...
  return {
    ...
    scrollToCurrent: () => {
      const s = get();
      scrollMatchIntoView(s.matches[s.currentIndex]);
    },
    ...
    replaceCurrent: () => {
      ...
      const fromBlock = order.get(match.blockId)!;
      // The inserted text's length, derived from the actual rewrite - in regex mode
      // the expansion of $1/$& differs from the replacement template's length.
      const insertedLength = text.length - block.text.length + (match.end - match.start);
      const fromOffset = match.start + insertedLength;
      let currentIndex = matches.findIndex((m) => {
        const mb = order.get(m.blockId)!;
    ...
    replaceAll: () => {
      const s = get();
      const blocks = useProjectStore.getState().blocks;
      const { edits, error } = replaceAllEdits(blocks, s.query, s.replacement, optionsOf(s));
      if (error) {
        set({ error });
        return;
      }
