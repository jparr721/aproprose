...
export type AiSettingsTarget = "key" | "model";
...
interface SettingsDialogState {
  open: boolean;
  tab: SettingsTab;
  aiTarget: AiSettingsTarget | null;
  setOpen: (open: boolean) => void;
  setTab: (tab: SettingsTab) => void;
  openWithTab: (tab: SettingsTab) => void;
  openAiSettings: (target: AiSettingsTarget) => void;
  clearAiSettingsTarget: () => void;
}
...
export const useSettingsDialogStore = create<SettingsDialogState>((set) => ({
  ...
  setTab: (tab) => set({ tab }),
  openWithTab: (tab) => set({ open: true, tab, aiTarget: null }),
  openAiSettings: (aiTarget) =>
    set({ open: true, tab: SETTINGS_TABS.AI, aiTarget }),
  clearAiSettingsTarget: () => set({ aiTarget: null }),
}));
